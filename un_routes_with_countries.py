...
def filter_routes(filename: str, output_file='data/new_routes_2.0'):
    ...
    with open(filename, mode='r') as main_file:
        ...
        with open(output_file, mode='w') as filter_data:
            ...
            #             assert len(row[2]) == 3 and len(row[4]) == 3
            #
            # def new_csv(file: str, country_dict: dict[str, list[tuple[str, str]]],
            #             output_file='data/new_routes_with_countries'):
            #     """
            #
            #     :param aiports:
            #     :param output_file:
            ...
            #                            'Guam',
            #                            'Guernsey',
            #                            'Holy See', 'Isle of Man', 'Jersey', 'Kosovo[1]', 'Martinique', 'Mayotte',
            #                            'Montserrat',
            #                            'New Caledonia', 'Niue', 'Northern Mariana Islands (Commonwealth of the)',
            #                            'occupied Palestinian territory, including east Jerusalem', 'Other',
            #                            'Pitcairn Islands',
            #                            'Puerto Rico', 'Réunion', 'Saba', 'Saint Barthélemy',
            #                            'Saint Helena, Ascension and Tristan da Cunha', 'Saint Martin',
            #                            'Saint Pierre and Miquelon',
            ...
            #             for row in reader:
            #                 first_date = datetime(2023, 2, 1).date()
            #                 if datetime.strptime(row[0], '%Y-%m-%d').date() >= first_date and row[
            #                     2] not in non_un_list and row[1] not in special_codes:
            #                     if row[1] == 'TR':
            #                         row[2] = 'Turkiye'
            #                     elif row[1] == 'CI':
            #                         row[2] = 'Cote d\'Ivoire'
